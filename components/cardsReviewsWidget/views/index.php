...
<?php foreach ($reviews as $review): ?>
    <div style="<?= $review->status == \app\models\Reviews::$STATUS['private'] ? 'background: #ffff66;' : '' ?>"
         class="block-reviews <?= !($settings['without_header'] ?? false) ? '' : 'without_header' ?>"
         data-reviews_id="<?= $review->id ?>" data-type="review">
            ...
            <a href="<?= Url::to(['post/index', 'url' => $review->post['url_name'], 'id' => $review->post['id']]) ?>">
                <div class="block-review-post" style="border-bottom: solid 1px #D3E4FF; padding: 20px;">
                    ...
                    <div class="block-info-review main-pjax">
                        ...
                        <div class="date-time-review"><?= implode(', ',
                                ArrayHelper::getColumn($review['post']['categories'], 'name')) ?></div>
                    </div>
                </div>
            </a>
        <?php endif; ?>
