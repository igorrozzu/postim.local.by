...
<?php elseif (($this->context->settings['show-pagination'] ?? false) && (($dataprovider->pagination->getLinks()['next'] ?? false) || ($dataprovider->pagination->getLinks()['prev'] ?? false))): ?>
    <div class="replace-block main-pjax">
        ...
        echo LinkPager::widget([
            ...
            'prevPageLabel' => false,
            'nextPageLabel' => false,
            'extraQuery' => (isset($settings['load-geolocation']) && is_array($settings['load-geolocation']) ? '&' . http_build_query(['load-geolocation' => $settings['load-geolocation']]) : ''),
        ]);
        ?>

        <?php if ($hrefNext = $dataprovider->pagination->getLinks()['next'] ?? false): ?>
            <a class="btn-show-more" style="margin-top: 0px"
               href="<?= $hrefNext ?><?= isset($settings['load-geolocation']) && is_array($settings['load-geolocation']) ? '&' . http_build_query(['load-geolocation' => $settings['load-geolocation']]) : '' ?>">
