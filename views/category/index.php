...
if (!$this->context->under_category) {
    ...
    $descriptionPage = \app\models\DescriptionPage::initMetaTags(function () use (
        $categoryText,
        $city_name,
        $totalCount,
        $h1_text
...
<div class="block-flex-white">
    <div class="block-content">
        <div class="block-sort">
            <a href="<?= Helper::createUrlWithSelfParams($selfParams, ['sort' => '_rating']) ?>"
               class="btn-sort <?= $sort == '_rating' ? 'active' : '' ?>"><span
                        class="under-line">По рейтингу</span></a>
            <a href="<?= Helper::createUrlWithSelfParams($selfParams, ['sort' => 'new']) ?>"
               class="btn-sort <?= $sort == 'new' ? 'active' : '' ?>"><span class="under-line">Новые</span></a>
            <?php if (Yii::$app->request->cookies->getValue('geolocation')): ?>
                <a href="<?= Helper::createUrlWithSelfParams($selfParams, ['sort' => 'nigh']) ?>"
                   class="btn-sort <?= $sort == 'nigh' ? 'active' : '' ?>"><span class="under-line">Рядом</span></a>
            <?php else: ?>
                <a style="display: none" href="<?= Helper::createUrlWithSelfParams($selfParams, ['sort' => 'nigh']) ?>"
                   class="btn-nigh btn-sort <?= $sort == 'nigh' ? 'active' : '' ?>"><span
                            class="under-line">Рядом</span></a>
...
<div class="block-content">
    ...
    <?php else: ?>
        <div class="container-message">
            <p>К сожалению, в <?= Yii::t('app/locativus', Yii::$app->city->getSelected_city()['name']) ?> на
                сайте <?= ucfirst(Yii::$app->getRequest()->serverName) ?> в категории <?= $categoryText ?> пока нет
                ничего. Если вы знаете о подходящем месте, добавьте его через кнопку <a class="add-place-href">"Добавить
                    место"</a>. За размещение новых мест на сайте мы начисляем бонусы пользователям.</p>
        </div>
    <?php endif; ?>
