<?php if ($postInfo->phones && is_array($postInfo->phones)): ?>
    <?php foreach ($postInfo->phones as $phone): ?>
        <div class="block-input-contact"><span class="container-img"><img src="/img/icon-phone-min.png"></span><input
                    ...
                    data-message="Некоректные данные для контактов" placeholder="Номер телефона"
...
<?php if ($postInfo->web_site): ?>
    <div class="block-input-contact"><span class="container-img"><img src="/img/icon-link-min.png"></span><input
                value="<?= $postInfo->web_site ?>" class="validator" data-error-parents="block-input-contact"
                data-message="Некоректные данные для контактов" placeholder="Ссылка на сайт" name="contacts[web_site]"
                data-regex="^((https?:\/\/)?([\da-z.-]+)\.([a-z\.]{2,6})([\/\w \.-]*)*\/?)|((https?:\/\/)?([\dа-я.-]+)\.([а-я\.]{2,6})([\/а-я \.-]*)*\/?)$">
        <div class="close-input-contact"></div>
    </div>
<?php endif; ?>
