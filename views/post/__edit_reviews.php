<div class="block-write-reviews active edit_reviews" data-post_id="<?= $reviews['post_id'] ?>"
                ...
                <div class="container-insert-photos">
                    <div class="container-photos-inputs">
                        <?php foreach ($reviews->gallery as $item): ?>
                            <input id="inputs_<?= md5($item['link']) ?>" style="display: none" name="reviews[photos][]"
                                   value="<?= $item['link'] ?>" type="text">
                        <?php endforeach; ?>
                    </div>
                    <div class="block-tmp-photos">
                        <?php foreach ($reviews->gallery as $item): ?>
                            <div id="<?= md5($item['link']) ?>" class="review-photo-tmp"
                                 style='background-image: url("<?= $item->getPhotoPath() ?>");'>
